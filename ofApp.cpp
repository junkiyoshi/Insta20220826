...
void ofApp::update() {
	...
	for (int k = 0; k < 5; k++) {
		...
		auto rotation_x = glm::rotate(glm::mat4(), ofMap(ofNoise(noise_seed.x, ofGetFrameNum() * 0.005), 0, 1, -PI * 2, PI * 2), glm::vec3(1, 0, 0));
		auto rotation_y = glm::rotate(glm::mat4(), ofMap(ofNoise(noise_seed.y, ofGetFrameNum() * 0.005), 0, 1, -PI * 2, PI * 2), glm::vec3(0, 1, 0));
		auto rotation_z = glm::rotate(glm::mat4(), ofMap(ofNoise(noise_seed.z, ofGetFrameNum() * 0.005), 0, 1, -PI * 2, PI * 2), glm::vec3(0, 0, 1));
	...
	for (int i = 0; i < this->draw_mesh.getNumVertices(); i++) {
		...
		ofColor color = ofColor(ofMap(this->param_list[i], 50, 0, 0, 239));
